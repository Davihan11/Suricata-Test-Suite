...
from util.suri_util import TestInfo
...
def test_rss(
    ...
    test_stages: List[float] = [1, 2, 3]

    stage_iter = multiplier_iterator_create(False, test_stages)
    for stage in stage_iter:
        logger.progress(
            f"Stage {stage} | param_file={request.config.getoption('--param-file')} | params={params}"
        )
        match stage:
            case 1:
                tester = TrexTestRun(
