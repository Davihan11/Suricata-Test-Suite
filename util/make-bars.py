...
def process_result_line(stats, graph_bar):
    ...
    max_speed = result_from_row.tx_bytes * BITS_PER_BYTE / (result_from_row.tx_time * BITS_PER_GIGABIT)

    print(f"Received packets total: {result_from_row.rx_packets}")
    print(f"Transmitted packets total: {result_from_row.tx_packets}")

    dropped_pkts: int = result_from_row.tx_packets - (result_from_row.rx_packets + result_from_row.rx_rte_flow_filtered_packets)
    print(f"Dropped packets total: {dropped_pkts}")

    packet_loss = 100*dropped_pkts/result_from_row.tx_packets if result_from_row.tx_packets != 0 else 0
...
def plot_graph(graph_bars, network_cards):
    ...
    ax.axhline(y=get_max_speed(graph_bars), color='green', linestyle='-.', linewidth=1, alpha=0.3)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(usage='%(prog)s -n|--network_cards "MLX5,ICE" [path to aggregated1.json] [path to aggregated2.json] ...')
