...
class ColoredTypeFormatter(logging.Formatter):
    ...
    Color is only applied when ``use_color`` is True, so the same formatter
    produces clean, escape-free output on non-TTY streams (pipes, ssh without
    a pty) while keeping the ``[LEVEL] | message`` layout identical.
    """
    ...
    def __init__(self, fmt=None, datefmt=None, style="%", use_color: bool = True):
...
def setup_logging(level: str | int = "INFO", log_file: str | None = None) -> None:
    ...
    # Only colorize the console when stdout is an interactive terminal; when
    # piped or over ssh without a pty, isatty() is False and output stays clean.
    console_formatter = ColoredTypeFormatter(
        fmt=CONSOLE_LOG_FORMAT, use_color=sys.stdout.isatty()
    )
