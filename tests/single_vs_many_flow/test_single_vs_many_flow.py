...
Compares single-flow vs many-flow throughput. A single 5-tuple hashes to one
NIC queue, so only one worker thread does the work (single-queue bottleneck);
many parallel flows spread across all queues. This isolates per-queue scaling.
"""
...
from util.pcap_gen import gen_single_flow, gen_many_flows
...
# Number of parallel flows for the "many" variant.
NUM_FLOWS = 100_000
PACKETS_PER_FLOW = 10
...
def test_single_vs_many_flow(
    ...
    if flow_mode == "single":
        pcap_path = gen_single_flow(tmp_dir, num_packets=1_000_000)
    else:
        pcap_path = gen_many_flows(
    ...
    for multiplier in mult_iter:
        logger.progress(
            f"multiplier {multiplier:.4f} | flow_mode={flow_mode} | params={params}"
        )
    ...
    if mult_iter.result is not None:
        logger.progress(
            f"Maximum multiplier found is: {mult_iter.result:.4f}. | flow_mode={flow_mode} | params={params}\n\n"
        )
    else:
        logger.progress(
            f"Enumeration complete. | flow_mode={flow_mode} | params={params}\n\n"
        )
