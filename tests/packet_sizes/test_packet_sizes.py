...
Tests Suricata throughput at fixed packet sizes (64B / 512B / 1400B / jumbo).
This isolates the low-level decode path (pps) from payload processing: small
packets are pps-bound, large packets are bandwidth-bound.
"""
...
from util.pcap_gen import gen_packet_sizes
...
# Total on-wire frame sizes to test. 64B is the Ethernet minimum (pps-bound),
# 1400B is typical MTU, 9000B is jumbo (bandwidth-bound).
PACKET_SIZES = [64, 512, 1400, 9000]
...
def test_packet_sizes(
    ...
    # Generate a pcap of the requested size into tmp/.
    tmp_dir = Path(request.node.path).parent / "tmp"
    pcaps = gen_packet_sizes(tmp_dir, sizes=[packet_size], packets_per_size=1000)
    pcap_path = pcaps[packet_size]
    ...
    for multiplier in mult_iter:
        logger.progress(
            f"multiplier {multiplier:.4f} | packet_size={packet_size} | params={params}"
        )
    ...
    if mult_iter.result is not None:
        logger.progress(
            f"Maximum multiplier found is: {mult_iter.result:.4f}. | packet_size={packet_size} | params={params}\n\n"
        )
    else:
        logger.progress(
            f"Enumeration complete. | packet_size={packet_size} | params={params}\n\n"
        )
