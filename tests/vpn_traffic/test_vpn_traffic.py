...
Tests the CPU cost of VPN/encapsulated traffic. Even when Suricata cannot
extract any useful data from the encrypted payload, it still has to parse the
outer headers and (for GRE/IPIP) the inner headers, which costs CPU.
Parametrised over the encapsulation type: GRE, IP-in-IP, IPsec-ESP, WireGuard.
"""
...
from util.pcap_gen import gen_vpn
...
# Encapsulation types to test. "gre" and "ipip" expose inner headers to
# inspection; "esp" and "wireguard" are opaque (encrypted) payloads.
ENCAPS = ["gre", "ipip", "esp", "wireguard"]
NUM_FLOWS = 1000
PACKETS_PER_FLOW = 10


@pytest.mark.parametrize("encap", ENCAPS)
...
def test_vpn_traffic(
    ...
    pcap_path = gen_vpn(
        tmp_dir, encap=encap, num_flows=NUM_FLOWS, packets_per_flow=PACKETS_PER_FLOW
    )
    ...
    for multiplier in mult_iter:
        logger.progress(
            f"multiplier {multiplier:.4f} | encap={encap} | params={params}"
        )
    ...
    if mult_iter.result is not None:
        logger.progress(
            f"Maximum multiplier found is: {mult_iter.result:.4f}. | encap={encap} | params={params}\n\n"
        )
    else:
        logger.progress(
            f"Enumeration complete. | encap={encap} | params={params}\n\n"
        )
