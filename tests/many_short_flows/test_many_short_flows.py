...
Tests Suricata behaviour under heavy flow churn: many short-lived flows
(<=5 packets each) that stress flow hash inserts, allocations and expiry
evictions. The flow table size and timeouts are tuned via param.py.
"""
...
# Number of distinct short flows to generate. Each flow carries <=5 packets.
NUM_FLOWS = 100_000
PACKETS_PER_FLOW = 5
...
def test_many_short_flows(
    ...
    for multiplier in mult_iter:
        logger.progress(
            f"multiplier {multiplier:.4f} | flows={NUM_FLOWS} pkts/flow={PACKETS_PER_FLOW} | params={params}"
        )
    ...
    if mult_iter.result is not None:
        logger.progress(
            f"Maximum multiplier found is: {mult_iter.result:.4f}. | flows={NUM_FLOWS} | params={params}\n\n"
        )
    else:
        logger.progress(
            f"Enumeration complete. | flows={NUM_FLOWS} | params={params}\n\n"
        )
