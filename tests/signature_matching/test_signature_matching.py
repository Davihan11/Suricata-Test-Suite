...
Tests signature/regex matching cost against the installed ruleset.
The traffic carries a distinctive "poison" string that matches a regex rule,
so the engine has to run the full ruleset against every packet.
"""
...
from util.pcap_gen import gen_signature_traffic
...
def test_signature_matching(
    ...
    pcap_path = gen_signature_traffic(
    ...
    for multiplier in mult_iter:
        logger.progress(
            f"multiplier {multiplier:.4f} | params={params}"
        )
