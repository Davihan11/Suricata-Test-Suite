...
usage(){
  ...
  echo "-sb   | --trex-stl-burst [PPS] [COUNT] to send a fixed burst of COUNT packets at PPS in STL mode. Defaults: 200000 PPS, 10000000 packets"
...
while [ "$#" -gt 0 ]; do
  case $1 in
    ...
    -sb | --trex-stl-burst)
        # accept 0, 1, or 2 numeric args; defaults are applied by pytest
        trex_stl_burst_set=true
        trex_stl_burst=""
        if [[ "$2" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
            trex_stl_burst="$2"
            shift
