...
usage(){
  ...
  echo "-sb   | --trex-stl-burst [PPS] [COUNT] to send a fixed burst of COUNT packets at PPS in STL mode. Defaults: $DEFAULT_STL_BURST_PPS PPS, $DEFAULT_STL_BURST_COUNT packets"
...
# Defaults for --trex-stl-burst (-sb) when given without arguments.
# Keep in sync with STL_BURST_DEFAULTS in conftest.py.
DEFAULT_STL_BURST_PPS=200000
DEFAULT_STL_BURST_COUNT=10000000
...
while [ "$#" -gt 0 ]; do
  case $1 in
    ...
    -sb | --trex-stl-burst)
        # accept 0 or 2 numeric args; with no args the defaults are passed
        extra_args+=(--trex-stl-burst)
        if [[ "$2" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
            extra_args+=("$2")
            shift
            if [[ ! "$2" =~ ^[0-9]+$ ]]; then
                echo "Error: --trex-stl-burst (-sb) expects 0 or 2 arguments: <PPS> <COUNT> (COUNT must be an integer)" >&2
                exit 1
            fi
            extra_args+=("$2")
            shift
        else
            # no args given: pass the defaults explicitly
            extra_args+=("$DEFAULT_STL_BURST_PPS" "$DEFAULT_STL_BURST_COUNT")
        fi
        shift
        ;;
