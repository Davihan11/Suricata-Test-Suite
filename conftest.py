...
def _validate_hugepages_option(config) -> None:
    """Validate the ``--suricata-hugepages`` option value up front.

    A mis-typed value (e.g. ``6X`` or ``abc``) would otherwise surface as a raw
    ``ValueError`` traceback from inside the session fixture. Validating here in
    ``pytest_configure`` turns it into a clean ``pytest.UsageError`` that names
    the offending value, before any fixture runs.
    """
    hugepages_opt = config.getoption("--suricata-hugepages")
    try:
        _parse_size_to_bytes(hugepages_opt)
