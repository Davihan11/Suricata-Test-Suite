...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store",
        help=(
                "Specify user that operates on the machine. "
            ),
        )
    parser.addoption(
        "--suricata-hugepages",
    ...
    parser.addoption(
        ...
        action="store",
        help=(
                "Specify path to default configuration to be used with Suricata. "
            ),
        )
    parser.addoption(
        "--collect-artifacts",
        ...
        action="store_true",
        help=(
                "Turn on collecting artifact data from individual tests. "
            ),
        )
    parser.addoption(
        "--test-comment",
        ...
        action="store",
        help=(
                "Set test-wide comment containg information about current test-run. "
            ),
        )
    parser.addoption(
        "--param-file",
        ...
        action="store",
        help=(
                "Specify name of file with parameters"
            ),
        )
    parser.addoption(
        "--traffic-duration",
    ...
    parser.addoption(
        ...
        action="store",
        help=(
            "Specify for how long to wait before measuring statistics"
        )
    )
    parser.addoption(
        "--pcap-file-stl",
...
def pytest_generate_tests(metafunc):
     if 'params' in metafunc.fixturenames:
        params = []
        capture_modes_in_run = get_capture_modes_in_run(metafunc.config.getoption("--param-file"))
        capture_modes = get_capture_modes(metafunc.config.getoption("--param-file"))

        for suricata_capture_mode in capture_modes_in_run:
            filter_suricata_capture_modes = [mode for mode in capture_modes if mode != suricata_capture_mode]

            raw_runs = parametrize_args(metafunc.config.getoption("--param-file"))
            if raw_runs:
                for run in raw_runs:
                    run = {k: v for k, v in run.items() if not any(mode in k for mode in filter_suricata_capture_modes)} # unwanted capture modes
                    if run:
                        params.append(run)
                        af_packet_get_queues_rx_descriptors(metafunc.config.getoption("--param-file"), params)

            assert params, "empty parameters, unable to set interface"

        params = [dict(t) for t in {tuple(d.items()) for d in params}] # remove duplicate
...
def send_pcap_to_trex(pcap_filename, request):
    ...
    os.system(f'rsync -z --checksum --update {pcap_filename} $(whoami)@{get_trex_internal(request)}.liberouter.org:/tmp/pcaps')
...
def bind(request):
    ...
    binds_info = executable.Tool(f"dpdk-devbind -s | grep {suri_interface_bind(request)[0]}",
                                        sudo=True,
    ...
    for pcie in pcies_to_vfio:
        if pcie in pcie_info:
            pcie_bind = executable.Tool(f"modprobe vfio-pci; echo 1 | sudo tee /sys/module/vfio/parameters/enable_unsafe_noiommu_mode; dpdk-devbind.py -b vfio-pci {suri_interface_bind(request)[0]}",
                                        sudo=True,
                                        executor=get_suri_executor(request),
                                        )
            pcie_bind.run()
...
def suricata_conf_file(request) -> ConfigBuilder:
    ...
    if request.config.getoption("--suricata-cfg"):
        builder = ConfigBuilder(editable_yaml, request.config.getoption("--suricata-cfg"))
    else:
...
def utilized_programs_info(request):
    get_dpdk_version_process = executable.Tool("pkg-config --modversion libdpdk",
                                               executor=get_suri_executor(request))
    dpdk_version, _ = get_dpdk_version_process.run()

    get_suricata_version_process = executable.Tool("suricata --build-info | head -1",
                                                   executor=get_suri_executor(request))
    suricata_version, _ = get_suricata_version_process.run()
    return {"dpdk_version" : dpdk_version.strip(), "suricata_version" : " ".join(suricata_version.split()[4:])}

@pytest.fixture(autouse=True)
def assert_available_machines(request) -> None:
    host_pcie_adress = suri_interface_bind(request)[0]

    process_get_pcie_match = executable.Tool(f"lshw -c network | grep -c {host_pcie_adress} > /tmp/pcie_count",
                                        sudo=True,
                                        executor=get_suri_executor(request),
                                        )
    ...
    assert stderr == "",  f"Error while gathering information about pcie interfaces, stderr: {stderr}"
...
def hugepages_allocated(request) -> bool:
    ...
    assert stderr == "",  f"Error while gathering information about allocated hugepages: {stderr}"
...
def check_hugepages(request) -> None:
    ...
    process_set_hugepages = executable.Tool(f"dpdk-hugepages.py --setup {request.config.getoption('--suricata-hugepages')}",
                                            sudo=True,
                                            executor=get_suri_executor(request),
                                            )
...
def import_module(param_file):
    ...
    spec = importlib.util.spec_from_file_location(module_name_of_param_file, module_path)
...
def parametrize_args(param_file):
    ...
    if hasattr(module, 'suri_yaml_params'):
        parametrize_args = [dict(zip(module.suri_yaml_params.keys(), combination)) for combination in product(*module.suri_yaml_params.values())]
...
def suri_conf(request, suricata_conf_file, get_test_name):
    return Suri_conf(conf_file=suricata_conf_file, server=get_host_internal(request), pcie=suri_interface_bind(request)[0], test_name=get_test_name)
...
def make_combinations_for_af_packet(queues, rx_descriptors):
    combinations = [ [i, j] for i in queues for j in rx_descriptors] # first thread, second rx_descriptors
    return combinations
...
def setup_af_packet(request):
    ...
    if "queues" in current_parameters.keys() and "rx_descriptors" in current_parameters.keys():
        interface = suri_interface_bind(request)[0]
        workers = current_parameters['queues']
        af_setup_prompts = [f"ip link set {interface} down", f"/usr/sbin/ethtool -L {interface} combined {workers}", f"/usr/sbin/ethtool -K {interface} rxhash on",
        f"/usr/sbin/ethtool -K {interface} ntuple on", f"ip link set {interface} up",
        ...
        f"/usr/sbin/ethtool -A {interface} rx off", f"/usr/sbin/ethtool -C {interface} adaptive-rx off adaptive-tx off rx-usecs 1",
        f"/usr/sbin/ethtool -G {interface} rx {current_parameters['rx_descriptors']}", f"/usr/sbin/ethtool -X {interface} hfunc toeplitz"]

        for af_setup_prompt in af_setup_prompts:
            set_af_packet = executable.Tool(f"{af_setup_prompt}",
                                            sudo=True,
                                            executor=get_suri_executor(request),
                                            )
            set_af_packet.run()
...
def af_packet_get_queues_rx_descriptors(param_file, params):
    ...
    if hasattr(module, 'afp_ethtool'):
        ...
        ).input_value(module.afp_ethtool).all()).replace("[", "").replace("]", "")
        ...
        assert queues_not_empty and rx_descriptors_not_empty # cannot be empty because of settings
